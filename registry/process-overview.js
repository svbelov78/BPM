(function () {
  ...
  // Process Details · Drawer 10 columns / Main content (1122:37241).
  function render(row, u) {
    ...
    const person = (name, position, training = false) => `<div class="pd-person"><div class="pd-person-line"><span class="pd-person-avatar">${icon('imgIcon24Person')}</span><p class="pd-person-name">${esc(name)}</p></div><div class="pd-person-copy"><p class="pd-person-position">${esc(position)}</p>${training ? tag('Обучение пройдено', 'green', 'imgIcon') : ''}</div></div>`;
