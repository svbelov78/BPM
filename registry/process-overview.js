(function () {
  ...
  function render(row, u) {
    ...
    const date = row.date ? new Date(`${row.date}T12:00:00`).toLocaleDateString('ru-RU') : '26.11.2018';
    ...
    const organization = `<article class="pd-overview-panel pd-organization">${field('Подразделение', `Блок «${esc(row.block || 'Транзакционный банкинг B2C')}»<br>Дивизион «${esc(row.division || 'Кредитные карты')}»`)}${field('Статус в реестре', `<span class="pd-field-status">${dot}<span>Подтвержден, создан ${esc(date)}</span></span>`)}${field('Статус исполнения', `<span class="pd-field-status">${dot}<span>${esc(row.status || 'Исполняется')}, установлен ${esc(date)}</span></span>`)}${field('Актуальность бизнес-описания', `<span class="pd-field-status">${dot}<span>Подписан, 01.02.2026<br>Иванов Сергей Иванович</span></span>`)}<p class="pd-small-muted">Процесс ПАО Сбербанк</p></article>`;
