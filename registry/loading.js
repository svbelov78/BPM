...
(() => {
  ...
  function tableRows(count, process) {
    const row = `<tr class="skeleton-row" aria-hidden="true"><td><div class="skeleton-table-description"><div class="skeleton-header">${shape('id')}${shape('status')}</div>${shape('title')}${shape('line')}</div></td><td><div class="skeleton-owner">${shape('avatar')}<div class="skeleton-owner-copy">${shape('line')}${shape('line')}</div></div></td><td><div class="skeleton-table-status">${shape('status')}${shape('date')}</div></td>${process?`<td><div class="skeleton-table-tags">${shape('tag')}${shape('tag')}</div></td>`:''}<td>${efficiency()}</td></tr>`;
